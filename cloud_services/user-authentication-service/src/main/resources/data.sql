...
INSERT INTO auth_users(id, email, password, user_type) SELECT 3, 'tenant2@test.ie', '$2a$10$bnC26zz//2cavYoSCrlHdecWF8tkGfPodlHcYwlACBBwJvcEf0p2G', 'PROPERTY_TENANT' WHERE NOT EXISTS(SELECT 1 FROM auth_users WHERE id=3)
INSERT INTO auth_users(id, email, password, user_type) SELECT 4, 'tenant3@test.ie', '$2a$10$bnC26zz//2cavYoSCrlHdecWF8tkGfPodlHcYwlACBBwJvcEf0p2G', 'PROPERTY_TENANT' WHERE NOT EXISTS(SELECT 1 FROM auth_users WHERE id=4)

INSERT INTO auth_users(id, email, password, user_type) SELECT 5, 'engineer1@test.ie', '$2a$10$bnC26zz//2cavYoSCrlHdecWF8tkGfPodlHcYwlACBBwJvcEf0p2G', 'MAINTENANCE_ENGINEER' WHERE NOT EXISTS(SELECT 1 FROM auth_users WHERE id=5)
